...
namespace smartHome{
    ...
    void SmartHome::printRoomStatus(std::string roomID){
        int ok=0;
        for(const auto& room : getRooms()){
        ...
        if(ok==0){
            std::cout<<"incorrect room id or inexistent room."<<std::endl;
        }
    }

    void SmartHome::printDeviceStatus(std::string deviceID){
        int ok=0;
        for(const auto& room : getRooms()){
            for(const auto& device : room.getDevices()){
                if(device->getId() == deviceID){
                    ok = 1;
                    std::cout << "The device " << device->getId() << " has status: ";
                    if(device->getStatus()){
                        std::cout << "active" << std::endl;
                    }
                    else{
        ...
        if(ok==0){
            std::cout << "The specific device was not found." <<std::endl;
        }
    }

    void SmartHome::printSensorStatus(std::string sensorID){
        int ok=0;
        for(const auto& room : getRooms()){
            for(const auto& device : room.getDevices()){
                for(const auto& sensor : device->getSensors()){
                    if(sensor->getID() == sensorID){
                        ok=1;
                        std::cout << "The sensor " << sensor->getID() << " is: ";
                        if(sensor->getIsFunctional()){
                            std::cout << "functional." << std::endl;
                        }
                        else{
        ...
        if(ok==0){
            std::cout << "The specific sensor was not found." <<std::endl;
        }
    }
    ...
    void printStatusPeriodically(SmartHome& home) {
        while (!stopPrinting) {
            {
                std::lock_guard<std::mutex> lock(gMutex);
                home.printStatus(); // Call the existing printStatus function
                std::cout << "\n";
            }
